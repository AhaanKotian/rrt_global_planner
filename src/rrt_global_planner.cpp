...
#include "rrt.h"
PLUGINLIB_EXPORT_CLASS(rrt_global_planner::Rrt, nav_core::BaseGlobalPlanner)
namespace rrt_global_planner
{
    ...
    Rrt::Rrt() : costmap_ros_(NULL), initialized_(false)
    {

    }

    Rrt::Rrt(std::string name, costmap_2d::Costmap2DROS* costmap_ros) : costmap_ros_(NULL), initialized_(false)
    {
    ...
    bool Rrt::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan)
    {
        ...
        // Create ROS node
        rosNode.reset(new ros::NodeHandle( "rrt" ));
        col_path = rosNode->advertise<nav_msgs::Path>("rrt_collision_checker",1);
        ...
        map_length = nx*ny;
        unsigned int mx,my,origin_idx;
        double last_x,last_y;
        costmap_->indexToCells(map_length-1,mx,my);
        costmap_->mapToWorld(mx,my,last_x,last_y);
        ...
        rrt_global_planner::init_line(&tree);
        
        Node* q1;
        // Publish all edges
        for (auto q : v)
        {
            //ROS_INFO_STREAM("q.idx = "<<q->idx<<" & source =  "<<source<<" & q.parent = "<<q->parent<< " & origin_idx = "<<origin_idx);
            if(q->parent != NULL)
            {
                ...
                rrt_global_planner::pub_line(&tree, &tree_pub, wx1, wy1, wx2, wy2);
            }
        }
    ...
    void Rrt::rrt(int start, int goal)
    {
        ROS_INFO_STREAM("rrt() start");
        ...
        ROS_INFO_STREAM("rrt() end");
    }

    Node* Rrt::sample()
    {
        std::uniform_int_distribution<int> distribution(0 , map_length);
        //ROS_INFO_STREAM("map_length = "<<map_length);
        while(true)
        {
    ...
    Node* Rrt::steer(Node* q_near, Node q_rand)
    {
        double step_size = M_SQRT2 * costmap_->getResolution() * 40;
    ...
    bool Rrt::collisionfree(Node q_near, Node q_new)
    {
        //ROS_INFO_STREAM("-------------------------------------------");
    ...
    double Rrt::dist_betn(Node q1, Node q2)
    {
        unsigned int mx,my;
        double wx1,wy1,wx2,wy2;
